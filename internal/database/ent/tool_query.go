...
type ToolQuery struct {
	...
	withDependencies *ToolDependencyQuery
	withFKs          bool
...
func (tq *ToolQuery) QueryRepository() *RepositoryQuery {
...
func (tq *ToolQuery) QueryDependencies() *ToolDependencyQuery {
	query := (&ToolDependencyClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *ToolQuery) FirstID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (tq *ToolQuery) FirstIDX(ctx context.Context) string {
...
func (tq *ToolQuery) OnlyID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (tq *ToolQuery) OnlyIDX(ctx context.Context) string {
...
func (tq *ToolQuery) IDs(ctx context.Context) (ids []string, err error) {
...
func (tq *ToolQuery) IDsX(ctx context.Context) []string {
	ids, err := tq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (tq *ToolQuery) WithRepository(opts ...func(*RepositoryQuery)) *ToolQuery {
	query := (&RepositoryClient{config: tq.config}).Query()
...
func (tq *ToolQuery) WithDependencies(opts ...func(*ToolDependencyQuery)) *ToolQuery {
	query := (&ToolDependencyClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *ToolQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tool, error) {
	...
	if query := tq.withRepository; query != nil {
		if err := tq.loadRepository(ctx, query, nodes, nil,
			func(n *Tool, e *Repository) { n.Edges.Repository = e }); err != nil {
			return nil, err
		}
	}
	if query := tq.withDependencies; query != nil {
		if err := tq.loadDependencies(ctx, query, nodes,
			func(n *Tool) { n.Edges.Dependencies = []*ToolDependency{} },
			func(n *Tool, e *ToolDependency) { n.Edges.Dependencies = append(n.Edges.Dependencies, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *ToolQuery) loadRepository(ctx context.Context, query *RepositoryQuery, nodes []*Tool, init func(*Tool), assign func(*Tool, *Repository)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Tool)
	for i := range nodes {
		if nodes[i].repository_tools == nil {
			continue
		}
		fk := *nodes[i].repository_tools
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "repository_tools" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (tq *ToolQuery) loadDependencies(ctx context.Context, query *ToolDependencyQuery, nodes []*Tool, init func(*Tool), assign func(*Tool, *ToolDependency)) error {
	...
	query.withFKs = true
	query.Where(predicate.ToolDependency(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(tool.DependenciesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.tool_dependencies
		if fk == nil {
			return fmt.Errorf(`foreign-key "tool_dependencies" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "tool_dependencies" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
