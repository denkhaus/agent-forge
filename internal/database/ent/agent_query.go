...
type AgentQuery struct {
	...
	withDependencies *AgentDependencyQuery
	withFKs          bool
...
func (aq *AgentQuery) QueryRepository() *RepositoryQuery {
...
func (aq *AgentQuery) QueryDependencies() *AgentDependencyQuery {
	query := (&AgentDependencyClient{config: aq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (aq *AgentQuery) Clone() *AgentQuery {
	...
	return &AgentQuery{
		...
		withDependencies: aq.withDependencies.Clone(),
...
func (aq *AgentQuery) WithRepository(opts ...func(*RepositoryQuery)) *AgentQuery {
	query := (&RepositoryClient{config: aq.config}).Query()
...
func (aq *AgentQuery) WithDependencies(opts ...func(*AgentDependencyQuery)) *AgentQuery {
	query := (&AgentDependencyClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withDependencies = query
	return aq
}
...
func (aq *AgentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Agent, error) {
	...
	if query := aq.withRepository; query != nil {
		if err := aq.loadRepository(ctx, query, nodes, nil,
			func(n *Agent, e *Repository) { n.Edges.Repository = e }); err != nil {
			return nil, err
		}
	}
	if query := aq.withDependencies; query != nil {
		if err := aq.loadDependencies(ctx, query, nodes,
			func(n *Agent) { n.Edges.Dependencies = []*AgentDependency{} },
			func(n *Agent, e *AgentDependency) { n.Edges.Dependencies = append(n.Edges.Dependencies, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AgentQuery) loadRepository(ctx context.Context, query *RepositoryQuery, nodes []*Agent, init func(*Agent), assign func(*Agent, *Repository)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Agent)
	for i := range nodes {
		if nodes[i].repository_agents == nil {
			continue
		}
		fk := *nodes[i].repository_agents
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "repository_agents" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (aq *AgentQuery) loadDependencies(ctx context.Context, query *AgentDependencyQuery, nodes []*Agent, init func(*Agent), assign func(*Agent, *AgentDependency)) error {
	...
	query.withFKs = true
	query.Where(predicate.AgentDependency(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(agent.DependenciesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.agent_dependencies
		if fk == nil {
			return fmt.Errorf(`foreign-key "agent_dependencies" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "agent_dependencies" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
