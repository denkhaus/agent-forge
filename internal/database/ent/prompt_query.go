...
type PromptQuery struct {
	...
	withDependencies *PromptDependencyQuery
	withFKs          bool
...
func (pq *PromptQuery) QueryRepository() *RepositoryQuery {
	query := (&RepositoryClient{config: pq.config}).Query()
...
func (pq *PromptQuery) QueryDependencies() *PromptDependencyQuery {
	query := (&PromptDependencyClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PromptQuery) FirstID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (pq *PromptQuery) FirstIDX(ctx context.Context) string {
...
func (pq *PromptQuery) OnlyID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (pq *PromptQuery) OnlyIDX(ctx context.Context) string {
...
func (pq *PromptQuery) IDs(ctx context.Context) (ids []string, err error) {
...
func (pq *PromptQuery) IDsX(ctx context.Context) []string {
	ids, err := pq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (pq *PromptQuery) Clone() *PromptQuery {
	...
	return &PromptQuery{
		...
		withRepository:   pq.withRepository.Clone(),
...
func (pq *PromptQuery) WithRepository(opts ...func(*RepositoryQuery)) *PromptQuery {
	query := (&RepositoryClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withRepository = query
	return pq
}
...
func (pq *PromptQuery) WithDependencies(opts ...func(*PromptDependencyQuery)) *PromptQuery {
	query := (&PromptDependencyClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *PromptQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Prompt, error) {
	var (
		...
		_spec       = pq.querySpec()
		loadedTypes = [2]bool{
			pq.withRepository != nil,
			pq.withDependencies != nil,
		}
	)
	if pq.withRepository != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := pq.withRepository; query != nil {
		if err := pq.loadRepository(ctx, query, nodes, nil,
			func(n *Prompt, e *Repository) { n.Edges.Repository = e }); err != nil {
			return nil, err
		}
	}
	if query := pq.withDependencies; query != nil {
		if err := pq.loadDependencies(ctx, query, nodes,
			func(n *Prompt) { n.Edges.Dependencies = []*PromptDependency{} },
			func(n *Prompt, e *PromptDependency) { n.Edges.Dependencies = append(n.Edges.Dependencies, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PromptQuery) loadRepository(ctx context.Context, query *RepositoryQuery, nodes []*Prompt, init func(*Prompt), assign func(*Prompt, *Repository)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*Prompt)
	for i := range nodes {
		if nodes[i].repository_prompts == nil {
			continue
		}
		fk := *nodes[i].repository_prompts
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "repository_prompts" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (pq *PromptQuery) loadDependencies(ctx context.Context, query *PromptDependencyQuery, nodes []*Prompt, init func(*Prompt), assign func(*Prompt, *PromptDependency)) error {
	...
	query.withFKs = true
	query.Where(predicate.PromptDependency(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(prompt.DependenciesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.prompt_dependencies
		if fk == nil {
			return fmt.Errorf(`foreign-key "prompt_dependencies" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "prompt_dependencies" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
