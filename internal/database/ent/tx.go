...
func (tx *Tx) init() {
	...
	tx.AgentDependency = NewAgentDependencyClient(tx.config)
	tx.Component = NewComponentClient(tx.config)
	tx.ComponentDependency = NewComponentDependencyClient(tx.config)
	tx.Fork = NewForkClient(tx.config)
	tx.LocalConfig = NewLocalConfigClient(tx.config)
	tx.Prompt = NewPromptClient(tx.config)
	tx.PromptDependency = NewPromptDependencyClient(tx.config)
	tx.Repository = NewRepositoryClient(tx.config)
	tx.SyncOperation = NewSyncOperationClient(tx.config)
	tx.Tool = NewToolClient(tx.config)
	tx.ToolDependency = NewToolDependencyClient(tx.config)
}
